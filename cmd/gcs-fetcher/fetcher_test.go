...
// fakeGCSProvider allows us to simulate errors when interacting with GCS.
type fakeGCSProvider struct {
	t       *testing.T
	objects map[string]fakeGCSResponse
}

func (f *fakeGCSProvider) newReader(context context.Context, bucket, object string) (io.ReadCloser, error) {
	f.t.Helper()
...
// fakeOSProvider raises errors if configures, otherwise simply passes
...
func (f *fakeOSProvider) rename(oldpath, newpath string) error {
	if f.errorsRename > 0 {
		f.errorsRename--
		return errRename
	}
...
func (f *fakeOSProvider) chmod(name string, mode os.FileMode) error {
	if f.errorsChmod > 0 {
		f.errorsChmod--
		return errChmod
	}
...
func (f *fakeOSProvider) create(name string) (*os.File, error) {
	if f.errorsCreate > 0 {
		f.errorsCreate--
...
func (f *fakeOSProvider) open(name string) (*os.File, error) {
	if f.errorsOpen > 0 {
		f.errorsOpen--
		return nil, errOpen
	}
...
func (*fakeOSProvider) removeAll(path string) error {
...
func buildManifestTestContext(t *testing.T) (tc *testContext, teardown func()) {
	...
	gf := &gcsFetcher{
		...
		os:          osProvider,
		destDir:     workDir,
		stagingDir:  filepath.Join(workDir, stagingFolder),
	...
	return &testContext{
		...
		func() {
			if err := os.RemoveAll(workDir); err != nil {
				t.Logf("Failed to remove working dir %q, continuing.", destDir)
			}
		}
}
